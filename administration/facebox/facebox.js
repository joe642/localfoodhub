...
(function($) {
  ...
  $.extend($.facebox, {
    ...
    loading: function() {
      ...
      $('#facebox').css({
        top:	(parent.location == window.location) ? getPageScroll()[1] + (getPageHeight() / 10) : getPageScroll()[1] - (getPageHeight() / 5), 
        left:	350
      }).show()
  ...
  function getPageScroll() {
   var xScroll, yScroll;
   if (parent.location == window.location) { // we're not in an iframe
    if (self.pageYOffset) {
   ...
   } else { // we're are in an iframe
    if (self.parent.pageYOffset) {
      yScroll = self.parent.pageYOffset;
      xScroll = self.parent.pageXOffset;
      ...
      xScroll = parent.document.documentElement.scrollLeft;
    } else if (parent.document.body) {// all other Explorers
      yScroll = parent.document.body.scrollTop;
      xScroll = parent.document.body.scrollLeft;	
    }
   }
  ...
  function getPageHeight() {
   ...
   if (parent.location == window.location) {  // we're not in an iframe
 	if (self.innerHeight) {	// all except Explorer
	  ...
	  //alert('windowHeight1 '+windowHeight);
    ...
    if (self.parent.innerHeight) {	// all except Explorer
      windowHeight = self.parent.innerHeight;
	  //alert('windowHeight1 '+windowHeight);
   ...
   //alert('windowHeight4 '+windowHeight);
   return windowHeight
  }
